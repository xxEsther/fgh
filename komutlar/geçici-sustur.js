...
module.exports.run = async (bot, message, args) => {
  ...
  let reason = args.slice(2).join(' ');
    ...
    let modlog = guild.channels.find('name', 'rn-log');
  if (!modlog) return message.reply('`rn-log` kanalını bulamıyorum.');
...
let muterole = message.guild.roles.find(r => r.name === "Susturulmuş");
  ...
  let mutetime = args[1];
  if(!mutetime) return message.reply("Doğru Kullanım: .sustur <oyuncu> <süre> <sebep>");
