...
exports.run = async (client, message, args) => {
  ...
  this.fighting = new Set();
        ...
        if (!opponent) return message.reply("<:ejeje:692311321680674877> Oynamak istediğin kişiyi etiketlemelisiniz.")
  
  if (opponent.bot) return message.reply('<:ejeje:692311321680674877> Botlar ile düello yapamazsın dostum.');
  if (opponent.id === message.author.id) return message.reply('<:ejeje:692311321680674877> Kendin ile düello atamazsın dostum.');
                ...
                await message.channel.send(`${opponent}, düello isteği geldi. Düello'yu kabul ediyor musun? (\`evet\` veya \`hayır\` olarak cevap veriniz.)`);
                                ...
                                if (!verification) {
                                        ...
                                        return message.channel.send(`<:ejeje:692311321680674877> Düello kabul edilmedi...`);
                                }
                        }
                        let userHP = 500;
                        let oppoHP = 500;
